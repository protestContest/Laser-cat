...
float min_y = 5;
float max_y = 80;
int min_freeze = 0; // 200
int max_freeze = 1; // 3000
float minimal_movement = 1;
float max_movement = 10;
...
void setup() {
  ...
  pinMode (7, OUTPUT);
  digitalWrite (7, HIGH);  // switch on  the laser
...
void loop() {
  ...
  while (dist(x_new_position, y_new_position, x_old_position, y_old_position) > max_movement) {
    x_new_position = random(min_x+minimal_movement, max_x-minimal_movement);
    y_new_position = random(min_y+minimal_movement, max_y-minimal_movement);
  }

  // x_new_position = min_x + (max_x - min_x)/2;
  // y_new_position += 1;
  // if (y_new_position > max_y) {
  //   y_new_position = min_y;
  // }
  // random_delay = 0;
  // movement_time = 10;
